...
const getBusDepartures = ({ stopCode, limit }) =>
  new Promise((resolve, reject) => {
    if (process.env.USE_TEST_DATA) {
      ...
      axios
        ...
        .then(response => {
          const parsedResponseData = JSON.parse(
            `[${response.data
              .replace(/]/g, '],')
              .replace(/\],$/, ']')
              .toString()}]`
          ...
          const busData = parsedResponseData.slice(1).sort((a, b) => {
            const x = a[5]
            const y = b[5]
          ...
          resolve({
            ...
            buses: busData
              .map(bus => ({
                ...
                service: bus[2],
                eta: Math.round(
                  ((Math.abs(new Date(bus[5] - Date.now())) % 86400000) % 3600000) / 60000
